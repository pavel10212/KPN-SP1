...
import {
  LoadingWrapper,
  useLoading,
} from "@/components/loading/loadingWrapper";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Registration failed: ${response.status} ${response.statusText}. ${
            errorBody || "No additional error information available."
          }`
        );
      }
      ...
      if (!dataResponse.ok) {
        ...
        throw new Error(
          `Failed to fetch data: ${dataResponse.status} ${
      ...
      if (!createResponse.ok) {
        ...
        throw new Error(
          `Failed to create bookings: ${createResponse.status} ${
              ...
              {["api_key"].map((field, index) => (
                ...
                >
                  <InputFieldWithTooltip
                    ...
                    onChange={handleChange}
                    error={errors[field]}
                    tooltipText={`Steps to get the API Key:\n1. Go to beds24.com\n2. Login to your account\n3. Navigate to Settings\n4. Click on Account Access\n5. Create API Key (Allow any IP, Do not allow Write)\n6. Copy the API key\n7. Paste it into the provided field`}
                  />
                </motion.div>
              ))}
              {["prop_key"].map((field, index) => (
                <motion.div
                  key={field}
                  variants={itemVariants}
                  initial="hidden"
                ...
                >
                  <InputFieldWithTooltip
                    ...
                    onChange={handleChange}
                    error={errors[field]}
                    tooltipText={`Steps to get the Prop Key:\n1. Go to beds24.com\n2. Login to your account\n3. Navigate to Settings\n4. Click on Apps & integrations\n5. Copy the propKey\n6. Paste it into the provided field`}
                  />
                </motion.div>
              ))}
