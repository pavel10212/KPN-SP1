...
const NotificationsSection = ({setError}) => {
    ...
    useEffect(() => {
        if (fcmEnabled && messages) {
            ...
            toast.success(messages.data.title, {description: messages.data.body});
        }
    }, [fcmEnabled, messages]);
        ...
        if (checked) {
            if (!isNotificationPermissionGranted) {
                ...
                if (permissionGranted) {
                    setFcmEnabled(true);
                    setNotifications((prev) => ({...prev, fcm: true}));
                } else {
                    toast.error("Notification permission denied. Please enable notifications in your browser settings.");
                    setNotifications((prev) => ({...prev, fcm: false}));
                }
            } else {
                setFcmEnabled(true);
                setNotifications((prev) => ({...prev, fcm: true}));
    ...
    const saveNotificationPreferences = async () => {
        ...
        try {
            const response = await fetch("/api/saveNotificationPreferences", {
                ...
                body: JSON.stringify({fcm: notifications.fcm, fcmToken: notifications.fcm ? fcmToken : null}),
            });
            ...
            if (result.fcmEnabled && fcmToken) {
                const topic = result.user.role === "admin" ? `team-${result.user.teamId}_admin` : `team-${result.user.teamId}_${result.user.role}`;
                ...
                if (!subscribeResponse.ok) throw new Error("Failed to subscribe to topic");
        ...
        <Card className="max-w-md">
            ...
            <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <Switch id="fcm-notifications" checked={notifications.fcm}
                            onCheckedChange={handleNotificationChange} aria-label="Firebase Push Notifications"/>
                </div>
                {isNotificationPermissionGranted &&
                    <p className="text-sm text-green-500">Notification access has already been granted.</p>}
                {!isNotificationPermissionGranted && notifications.fcm &&
                    <p className="text-sm text-red-500">Notification permission is required. Please enable notifications
                        in your browser settings.</p>}
            </CardContent>
            <CardFooter>
                <Button onClick={saveNotificationPreferences} disabled={isNotificationLoading || isTokenLoading}
