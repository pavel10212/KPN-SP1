                ...
                <header className="bg-white rounded-xl shadow-lg p-6 mb-8 max-w-2xl">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        {session?.user?.role === "admin" && (
                            ...
                            >
                                <Refresh className={`mr-1 sm:mr-2 ${isRefreshing ? "animate-spin" : ""}`}
                                         fontSize="small"/>
                                <span
                                    className="hidden sm:inline">{isRefreshing ? "Refreshing..." : "Pull Bookings"}</span>
                                <span className="sm:hidden">{isRefreshing ? "..." : "Pull"}</span>
                            </button>
                        )}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <BookingInfo icon={Hotel} color="blue" label="Total Bookings" count={bookings.length}/>
                        <BookingInfo icon={TrendingUp} color="green" label="Upcoming Bookings"
                                     count={bookings.filter(b => new Date(b.firstNight) > new Date()).length}/>
                    </div>
                </header>
