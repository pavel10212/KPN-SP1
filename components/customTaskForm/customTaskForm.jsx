...
const CustomTaskForm = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to submit the form: ${response.status} ${response.statusText}`
        );
      }

      try {
        const sendNotification = await fetch(
          "/api/sendCustomTaskNotification",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!sendNotification.ok) {
          throw new Error(
            `Failed to send notification: ${sendNotification.status} ${sendNotification.statusText}`
          );
        }
      } catch (error) {
