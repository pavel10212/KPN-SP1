...
const ClientUsers = ({users, user}) => {
    ...
    useEffect(() => {
        const handleResize = () => {
            if (containerRef.current) {
                const newHeight = window.innerHeight - containerRef.current.getBoundingClientRect().top - 200;
    ...
    const columns = [
        ...
        },
        {
            ...
            minWidth: 150,
            renderCell: (params) => (
                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getRoleColor(params.value)}`}>
          {params.value}
        </span>
            ),
        },
        ...
        <div className="p-0 sm:p-6">
            <div className="bg-white rounded-xl shadow-md p-6" ref={containerRef}>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Your Teammates</h2>
                    {user.role === "admin" && (
