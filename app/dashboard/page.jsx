...
export default async function Dashboard() {
    ...
    const filterBookingsByDate = (bookings, dateField) =>
        bookings.filter(booking => new Date(booking[dateField]).setHours(0, 0, 0, 0) === today.getTime());
    ...
    const cards = [
        {id: 1, title: "Total Team Members", number: teamCount.length, icon: "👥"},
        {id: 2, title: "Check-ins Today", number: bookingsStartingToday.length, icon: "🛎️"},
        {id: 3, title: "Check-outs Today", number: bookingsEndingToday.length, icon: "🔑"}
    ];
        ...
        return (
            <div className="min-h-screen p-0 pb-2 sm:p-6">
                ...
                <div className="space-y-6">
                    <BookingToday bookings={check_ins} isMaid={teamMember.role === "Maid"}
                                  checkInOrOut="Upcoming Check-Ins" excludeField="lastNight"/>
                    <BookingToday bookings={check_outs} isMaid={teamMember.role === "Maid"}
                                  checkInOrOut="Upcoming Check-outs" excludeField="firstNight"/>
                </div>
            </div>
        );
